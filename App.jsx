...
export default function App() {
  ...
  // Estado de navegación: es de cada persona/dispositivo, no se sincroniza.
    ...
    <div style={S.page}>
      ...
      <div className="appGrid">
        <div className="colLeft">
          {/* Local name + objetivo + franja horaria */}
          <div style={S.card}>
          ...
          {/* Calendario compartido de horarios */}
          <div style={S.card}>
            ...
            <div style={{ marginTop: 10 }}>
              <SharedCalendar
                ...
                horaInicio={local.horaInicio} horaFin={local.horaFin}
        ...
        <div className="colRight">
          <div className="stickyGroup">
            {/* Editor de horario del día seleccionado */}
            <TurnoEditorCard
              vendedor={vActivo}
              vendedores={local.vendedores}
              local={local}
...
// Marca el día actual y señala los días con huecos de cobertura sin cubrir.
function SharedCalendar({ year, month, nDias, leadBlanks, prefix, vendedores, horaInicio, horaFin, diaSeleccionado, onSelectDia }) {
  const cells = [];
  ...
  for (let d = 1; d <= nDias; d++) {
    ...
    const tieneHueco = huecosDelDia(vendedores, key, horaInicio, horaFin).length > 0;
    ...
    cells.push(
      <button key={d} onClick={() => onSelectDia(d)} style={cellStyle}>
        <span style={hoy ? S.dayNumRowHoy : S.dayNumRow}>
          {d}
...
function VendedorEditor({ v, prefix, onChange, onCopiarMesAnterior, onRemove, canRemove, horasCalc }) {
  ...
  const handleCopiar = () => {
    ...
    setCopiadoMsg(n > 0 ? `Se copiaron ${n} días` : "No hay días el mes pasado para copiar");
      ...
      {v.modo === "calendario" ? (
        <>
          <div style={S.rowBetween3}>
            <span style={S.miniStat}>{diasMarcados} días marcados · {fmt(horasCalc, 1)} hs este mes</span>
            <button onClick={handleCopiar} style={S.copyBtn}>Copiar mes anterior</button>
          </div>
          {copiadoMsg && <div style={S.copiadoMsg}>{copiadoMsg}</div>}
        </>
      ) : (
        <div style={S.twoCol}>
          <label style={S.field}>
...
/* ---------- estilo: ranking VDH, en azul petróleo ---------- */

const ACCENT = "#2C6E71";
const ACCENT_SOFT = "#E3EFEE";
const INK = "#232823";
const SUB = "#8C9089";
const BG = "#EDF1F0";
const CARD = "#FFFFFF";
...
const GOOD = "#2C6E71";
const DANGER = "#A97A66";
const DANGER_BG = "#F2E9E4";
const WARN = "#C98A2E";
const WARN_BG = "#F5EDDD";

const S = {
  page: {
    minHeight: "100vh", background: BG, color: INK,
    fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Segoe UI', sans-serif",
    padding: "24px 20px 48px", maxWidth: 1080, margin: "0 auto",
  },
  ...
  headerBand: {
    background: CARD, borderRadius: 18, padding: "14px 15px", marginBottom: 12,
    boxShadow: "0 1px 2px rgba(0,0,0,0.04), 0 8px 20px -12px rgba(0,0,0,0.08)",
  },
  headerTop: { display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 12 },
  headerRight: { display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 3 },
  logoRow: { display: "flex", alignItems: "center", gap: 9 },
  logoMark: {
    width: 34, height: 34, borderRadius: 9, background: ACCENT, color: "#fff",
    display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, letterSpacing: "-0.02em",
  },
  brandTitle: { fontSize: 16, fontWeight: 800, letterSpacing: "-0.01em", lineHeight: 1.15 },
  brandSubtitle: { fontSize: 9.5, fontWeight: 700, letterSpacing: "0.05em", color: SUB, marginTop: 1 },
  savedPill: {
  ...
  localDropdownWrap: { position: "relative" },
  localDropdownBtn: {
    width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between",
    background: BG, border: `1px solid ${LINE}`, borderRadius: 12, padding: "10px 12px",
    cursor: "pointer",
  },
  ...
  card: {
    background: CARD, borderRadius: 18, padding: 15, marginBottom: 12,
    boxShadow: "0 1px 2px rgba(0,0,0,0.04), 0 8px 20px -12px rgba(0,0,0,0.08)",
  },
  ...
  nameInputSm: {
    ...
    border: `1.5px solid ${LINE}`, borderRadius: 9, background: "#FAFAFB",
    padding: "8px 10px",
  },
  ...
  input: {
    ...
    color: INK, background: "#FAFAFB", outline: "none", width: "100%", boxSizing: "border-box",
  },
  iconGhost: {
    width: 28, height: 28, borderRadius: 8, border: "none", background: DANGER_BG, color: DANGER,
    display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0,
  },
  sectionLabel: { fontSize: 12, color: SUB, fontWeight: 600, margin: "2px 0 8px" },
  ...
  smallGhostBtn: {
    fontSize: 11.5, fontWeight: 600, color: SUB, background: "#F5F5F7", border: "none",
  ...
  navBtn: {
    width: 28, height: 28, borderRadius: 999, border: `1px solid ${LINE}`, background: "#FAFAFB",
    color: INK, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer",
  },
  navBtnSm: {
    width: 22, height: 22, borderRadius: 999, border: `1px solid ${LINE}`, background: "#FAFAFB",
  ...
  monthLabel: { fontSize: 14, fontWeight: 700, minWidth: 120, textAlign: "center" },
  ...
  dayCell: {
    minHeight: 62, border: "none", borderRadius: 10, background: "#F5F5F7", color: INK,
    cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "stretch",
    justifyContent: "flex-start", padding: "4px 3px", gap: 2, textAlign: "left", boxSizing: "border-box",
  },
  dayCellSelected: {
    minHeight: 62, border: `1.5px solid ${ACCENT}`, borderRadius: 10, background: ACCENT_SOFT, color: INK,
    cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "stretch",
    justifyContent: "flex-start", padding: "3.5px 2.5px", gap: 2, textAlign: "left", boxSizing: "border-box",
  },
  dayCellHoy: {
    minHeight: 62, border: `1.5px solid ${ACCENT}`, borderRadius: 10, background: "#F5F5F7", color: INK,
    cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "stretch",
    justifyContent: "flex-start", padding: "3.5px 2.5px", gap: 2, textAlign: "left", boxSizing: "border-box",
  },
  dayNumRow: { fontSize: 10.5, fontWeight: 700, color: INK, padding: "0 2px", display: "flex", alignItems: "center", gap: 3 },
  dayNumRowHoy: { fontSize: 10.5, fontWeight: 800, color: ACCENT, padding: "0 2px", display: "flex", alignItems: "center", gap: 3 },
  turnoBar: {
    borderRadius: 4, padding: "1px 3px", fontSize: 7.5, fontWeight: 700, color: "#fff",
    whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", lineHeight: 1.4,
  },
  moreBadge: { fontSize: 7.5, fontWeight: 700, color: SUB, padding: "0 3px" },
  ...
  modeBtn: {
    flex: 1, fontSize: 12, fontWeight: 600, color: SUB, background: "#F5F5F7", border: "none",
    borderRadius: 9, padding: "7px 0", cursor: "pointer",
  },
  modeBtnActive: {
    flex: 1, fontSize: 12, fontWeight: 700, color: "#fff", background: INK, border: "none",
    borderRadius: 9, padding: "7px 0", cursor: "pointer",
  },
  rowBetween3: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, marginTop: 6, flexWrap: "wrap" },
  copyBtn: {
    fontSize: 11, fontWeight: 700, color: ACCENT, background: ACCENT_SOFT, border: "none",
    borderRadius: 7, padding: "5px 9px", cursor: "pointer", whiteSpace: "nowrap", marginTop: 8,
  },
  copiadoMsg: { fontSize: 11, color: ACCENT, fontWeight: 600, marginTop: 5 },
  note: { fontSize: 11.5, color: DANGER, lineHeight: 1.4 },
  notePlain: { fontSize: 11.5, color: SUB, lineHeight: 1.4 },
  ...
  turnoSelect: {
    ...
    color: INK, background: "#FAFAFB", outline: "none", minWidth: 0,
  },
...
const CSS = `
  ...
  .appGrid { display: flex; flex-direction: column; }
  .colLeft, .colRight { display: flex; flex-direction: column; }
